...
e.stopPropagation()});$('.menu-item-has-children').on('click',function(){$(this).find('.sub-menu').stop().slideToggle();$(document).click(function(e){e.stopPropagation();if($('.menu-item-has-children').has(e.target).length===0){$('.sub-menu').hide()}})})});$(function(){var $win=$(window);$win.on('scroll',function(){if($win.width()>=769){var $navbar=$('.header__navbar');if($win.scrollTop()>90){$navbar.addClass('fixed')}else{$navbar.removeClass('fixed')}}})});$(function(){const scrollLink=$('.header__information a[href^="#"], nav a[href^="#"], .hero a[href^="#"], .procedures a[href^="#"]').not($('.menu-item-has-children > a'));scrollLink.on('click',function(e){e.preventDefault();if($(this).length){var id=$(this).attr('href');var targetOffset=$(id).position().top;const menuHeight=$('.header').innerHeight();$('html, body').stop().animate({scrollTop:targetOffset-menuHeight},800)}else{$('html, body').stop().animate({scrollTop:0},800)}});$('.menu-item-has-children > a').click(function(e){e.preventDefault()})});$(function(){$('.doubts__container__content__card__body').hide();$('.doubts__container__content__card__header').click(function(){$(this).next().slideToggle();$(this).parent().siblings().children().next().slideUp();return!1})});$('.highlight_categories__row__card').click(function(){var css="highlight_categories__row__card--active";var item=$(this);var items=$('.highlight_categories__row__card');var background=$('.highlight_categories__background img');if(!item.hasClass(css)){items.removeClass(css);item.addClass(css);background.fadeTo(300,0.1,function(){$(this).attr('src',item.find('img').attr('src'));$(this).fadeTo(300,1)})}});/*!
 * sweetalert2 v7.0.8
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/elocation/source/Support/Sender.php';$('#form').submit(function(){$(".contact__row__form__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__row__form__status').append(data);setTimeout(function(){$(".contact__row__form__status").addClass("d-none")},3000)},error:function(){$('.contact__row__form__status').append('Erro')}});return!1})})