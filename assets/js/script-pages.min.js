...
e.stopPropagation()});$('.menu-item-has-children').on('click',function(){$(this).find('.sub-menu').stop().slideToggle();$(document).click(function(e){e.stopPropagation();if($('.menu-item-has-children').has(e.target).length===0){$('.sub-menu').hide()}})})});$(function(){var $win=$(window);$win.on('scroll',function(){if($win.width()>=769){var $navbar=$('.header__navbar');if($win.scrollTop()>90){$navbar.addClass('fixed')}else{$navbar.removeClass('fixed')}}})});$(function(){$('.doubts__container__content__card__body').hide();$('.doubts__container__content__card__header').click(function(){$(this).next().slideToggle();$(this).parent().siblings().children().next().slideUp();return!1})});/*!
 * sweetalert2 v7.0.8
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href").split("contato/",1).reverse().join('');const file='wp-content/themes/elocation/source/Support/Sender.php';$('#form').submit(function(){$(".contact__content__row__form__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__content__row__form__status').append(data);setTimeout(function(){$(".contact__content__row__form__status").addClass("d-none")},3000)},error:function(){$('.contact__content__row__form__status').append('Erro')}});return!1})});$(function(){$('.single__content__container__row__image__list__item').click(function(){var item=$(this);var items=$('.single__content__container__row__image__list__item');var thumb=$('.single__content__container__row__image__cover__thumb__image');if(!item.hasClass('active')){items.removeClass('active');item.addClass('active');thumb.fadeTo(200,0.1,function(){$(this).attr('src',item.find('img').attr('src'));$(this).fadeTo(200,1)})}})})